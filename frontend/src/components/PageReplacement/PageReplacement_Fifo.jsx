...
const FIFOPageReplacement = () => {
  ...
  // Initialize frames with nulls when frame count changes
  useEffect(() => {
    resetAnimation();
  }, [frameCount]);
  ...
  const processStep = (step) => {
    ...
    if (!newFrames.includes(page)) {
      ...
      if (newFrames.filter(f => f !== null).length < frameCount) {
        // If we have empty frames, find the first empty one
        const emptyIndex = newFrames.indexOf(null);
        if (emptyIndex !== -1) {
          newFrames[emptyIndex] = page;
          newFifoQueue.push({ page, index: emptyIndex });
          replacedFrameIndex = emptyIndex;
        }
      } else {
        // All frames are full, replace the oldest one (FIFO)
        const oldest = newFifoQueue.shift();
        replacedFrameIndex = oldest.index;
        newFrames[replacedFrameIndex] = page;
        newFifoQueue.push({ page, index: replacedFrameIndex });
      }
    }
    ...
    newHistory[step] = {
      ...
      frames: [...newFrames],
      pageFault,
      newFrameIndex: replacedFrameIndex
    };
  ...
  }, [isRunning, currentStep, animationSpeed, pageSequence.length]);
  ...
  return (
    <div className="bg-gray-900 min-h-screen text-white p-10">
      <div className="max-w-5xl mx-auto space-y-10">
        <h1 className="text-3xl font-bold text-blue-400">FIFO Page Replacement Algorithm</h1>
                        ...
                        {item.frames.map((frame, frameIdx) => (
                          <div
                            ...
                            className={`px-2 py-1 rounded text-center text-sm ${
                              frame === null
                                ? 'bg-gray-700'
                                : frameIdx === item.newFrameIndex && item.pageFault
