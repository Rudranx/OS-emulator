...
const LRUPageReplacement = () => {
  ...
  const processStep = (step) => {
    ...
    // Fixed: Create a proper copy of the recentUseMap
    const recentUseMap = step > 0 ? new Map(newHistory[step - 1].recentUseMap) : new Map();
    let pageFault = false;
    ...
    if (!currentFrames.includes(page)) {
      ...
      if (emptyIndex !== -1) {
        ...
        // Fixed: Properly identify the LRU page
        let lruPage = null;
        let lruTimestamp = Infinity;

        // Find the page with the oldest timestamp
        for (const frameValue of currentFrames) {
          if (frameValue !== null) {
            const timestamp = recentUseMap.get(frameValue);
    ...
    recentUseMap.set(page, step);

    newHistory[step] = {
      page,
      frames: [...currentFrames],
      pageFault,
      recentUseMap: new Map(recentUseMap)
    };
  ...
  const pageFaults = replacementHistory.filter(item => item.pageFault).length;
  const pageFaultRate = replacementHistory.length > 0 
    ? ((pageFaults / replacementHistory.length) * 100).toFixed(2) 
    : "0.00";

  return (
    <div className="bg-gray-900 min-h-screen text-white p-10">
      <div className="max-w-5xl mx-auto space-y-10">
        <h1 className="text-3xl font-bold text-blue-400">LRU Page Replacement Algorithm</h1>
